#!/usr/bin/env python3
"""Cut a Sentinel-2 window out of the public AWS COG archive.

    python3 fetch_s2_window.py --lat -35.13 --lon 149.53 --km 14 \
        --year 2024 --months 5,6,7,8 --band B02 --name capital

Reads only the window it needs -- the source COGs are 10980 x 10980, about
240 MB a band -- and writes a GeoTIFF plus a JSON sidecar carrying the
acquisition time, sun geometry and per-band satellite view angles taken from
the scene's own granule metadata.

What it handles that a naive fetch does not:

  * MGRS tiles filed one latitude band away from where a point converts, and
    windows that would be silently clipped to the edge of the wrong tile
  * scene selection screened on the 20 m SCL band over *this window*, rather
    than on tile-level cloud cover describing 12000 km2
  * ranking the survivors by sun elevation, which is what matters for anything
    shadow-based (--prefer clearest inverts that for reflectance work)

No credentials: the bucket is public.
Needs: pip install rasterio numpy mgrs utm
"""
...
def write_json_sidecar(path_json: str, tif_name: str, data, transform,
                       epsg: int, meta: dict, azimuth: dict, zenith: dict,
                       sensing: str, nodata, band: str) -> None:
    """Everything needed to interpret the window, in one JSON file."""
    ...
    def corner(row, col):
        e, n = ox + col * RESOLUTION_M, oy - row * RESOLUTION_M
        lat, lon = utm.to_latlon(e, n, zone, northern=northern, strict=False)
        return {"easting": round(e, 1), "northing": round(n, 1),
    ...
    doc = {
        ...
        "acquired": sensing or meta["datetime"],
        "tile": meta["tile"],
        "size": {"width": width, "height": height},
        "resolution_m": RESOLUTION_M,
        "crs": f"EPSG:{epsg}",
        "utm_zone": f"{zone}{'N' if northern else 'S'}",
        "origin": {"easting": ox, "northing": oy},
        "nodata": nodata,
        "cloud": {"tile_percent": meta["cloud"],
                  "window_fraction": round(meta.get("window_cloud", 0.0), 4)},
        "sun": {"azimuth": meta["sun_azimuth"], "elevation": meta["sun_elevation"]},
        "satellite_azimuth": azimuth,
        "satellite_zenith": zenith,
        "corners": {"ul": corner(0, 0), "ur": corner(0, width),
                    "ll": corner(height, 0), "lr": corner(height, width),
                    "center": corner(height / 2, width / 2)},
        "source": f"{BUCKET}/{meta['path']}",
    }
    with open(path_json, "w") as fh:
        json.dump(doc, fh, indent=2)


def write_gdalinfo_sidecar(path_txt: str, tif_name: str, data, transform,
                           epsg: int, meta: dict, azimuth: dict, zenith: dict,
                           sensing: str, nodata) -> None:
    """The same facts in `gdalinfo` text layout, for tools that parse that."""
    height, width = data.shape
    zone = epsg % 100
    northern = 32600 < epsg < 32700
...
def main(argv=None) -> int:
    ...
    p.add_argument("--prefer", default="lowest-sun",
                   choices=["lowest-sun", "clearest"],
                   help="lowest-sun for shadow-based work (default); clearest "
                        "for reflectance, indices and classification")
    p.add_argument("--sidecar", default="json",
                   choices=["json", "gdalinfo", "both"],
                   help="metadata sidecar format to write alongside the GeoTIFF")
    ...
    # Order matters: a 26 degree sun throws twice the shadow a 45 degree one
    # does, which is everything for shadow-based work and usually unwanted for
    # reflectance work, where the clearest scene wins instead.
    if args.prefer == "clearest":
        candidates.sort(key=lambda m: (m["cloud"], -m["sun_elevation"]))
    else:
    ...
    epsg = int(profile["crs"].to_epsg())
    with rasterio.open(tif, "w", **profile) as dst:
        dst.write(data, 1)

    written = [tif]
    if args.sidecar in ("json", "both"):
        js = os.path.join(args.out, f"{args.name}.json")
        write_json_sidecar(js, os.path.basename(tif), data, transform, epsg,
                           best, azimuth, zenith, sensing, nodata, args.band)
        written.append(js)
    if args.sidecar in ("gdalinfo", "both"):
        txt = os.path.join(args.out, f"{args.name}_meta.txt")
        write_gdalinfo_sidecar(txt, os.path.basename(tif), data, transform,
                               epsg, best, azimuth, zenith, sensing, nodata)
        written.append(txt)

    valid = data != (nodata if nodata is not None else 0)
    centre = utm.to_latlon(transform.c + data.shape[1] / 2 * RESOLUTION_M,
                           transform.f - data.shape[0] / 2 * RESOLUTION_M,
                           epsg % 100, northern=32600 < epsg < 32700, strict=False)
    print(f"window   {data.shape[1]}x{data.shape[0]} px centred "
          f"{centre[0]:.5f},{centre[1]:.5f}  {valid.sum() / data.size:.1%} valid, "
