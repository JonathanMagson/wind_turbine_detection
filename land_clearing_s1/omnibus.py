"""Sequential omnibus change detection for dual-pol Sentinel-1 on Google Earth Engine.

Verbatim port of the algorithm published in the Earth Engine Community tutorial
"Detecting Changes in Sentinel-1 Imagery" (parts 2-3) by Mort Canty, also
distributed as the ``eesar`` package in https://github.com/mortcanty/eesarseq.

Tutorial source: https://github.com/google/earthengine-community
  tutorials/detecting-changes-in-sentinel-1-imagery-pt-3  (Apache License 2.0)
...
Inputs must be a list of *linear power* dual-pol (VV, VH) images multiplied by
the equivalent number of looks (ENL). See ``s1.py``.

``change_maps`` returns a dictionary of:
  ...
  bmap  - one band per interval, coded 0 = no change, 1 = positive definite
          (backscatter increase), 2 = negative definite (backscatter decrease),
          3 = indefinite
"""
